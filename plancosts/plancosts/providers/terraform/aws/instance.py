...
def _extract_tuple_from_any(*args) -> Tuple[str, str, Dict[str, Any], Optional["ResourceData"]]:
    """
    Accept any of the 3 constructor calling styles and normalize to:
      (address, region, raw_values, rd)
    Supported call patterns:
      1) (address, region, raw_values, rd)  # preferred
      2) (address, region, raw_values)      # ok
      3) (rd, usage_rd)                     # legacy (ignore usage_rd)
    """
    # Pattern 1 or 2: first arg is address string
...
def _bd_volume_size(values: Dict[str, Any]) -> Decimal:
    # Instance block devices use 'volume_size'; default to 8 GB
...
class _BlockDeviceGB(BaseAwsPriceComponent):
    ...
    Attribute: volumeApiName (mapped from 'volume_type')
    Quantity: volume_size (default 8)
    """
...
class _BlockDeviceIOPS(BaseAwsPriceComponent):
    ...
    def __init__(self, name: str, resource: "AwsBlockDevice"):
        ...
        self.default_filters = [
            Filter(key="servicecode", value="AmazonEC2"),
            # Catalog exposes PIOPS under System Operation
            Filter(key="productFamily", value="System Operation"),
            # Keep family constraint to io1/io2; DO NOT add usagetype here
            Filter(key="volumeApiName", value=_bd_volume_type(v)),
        ]
        self.value_mappings = [
            ValueMapping(from_key="volume_type", to_key="volumeApiName"),
        ]
        # Price filter must use the exact unit string from your catalog
        self.set_price_filter({
            "purchaseOption": "on_demand",
            "unit": "IOPS-Mo",
            # descriptionRegex not required; leaving it out avoids over-filtering
        })
...
class _ComputeHours(BaseAwsPriceComponent):
    ...
    Parity with computeCostComponent for on-demand:
      ...
      Unit: "hours"
      Hourly quantity: 1
      ProductFilter: Compute Instance, instanceType, tenancy (mapped),
                     OS=Linux, preInstalledSw=NA, capacitystatus=Used
      PriceFilter: purchaseOption=on_demand
    """
    def __init__(self, resource: "AwsInstance"):
        ...
        self.default_filters = [
            ...
            Filter(key="tenancy", value="Shared"),  # overridden by mapping if dedicated
        ]
        self.value_mappings = [
...
class AwsInstance(BaseAwsResource):
    ...
    # NOTE: we accept flexible signatures via *args so the parser never falls back.
    def __init__(self, *args, **_kwargs):
        address, region, raw_values, _rd = _extract_tuple_from_any(*args)
        super().__init__(address, region, raw_values)

        # Top-level compute hours component (on-demand)
        self._set_price_components([_ComputeHours(self)])

        subs: List[BaseAwsResource] = []

        # root_block_device: treat as single nested object; always create one
