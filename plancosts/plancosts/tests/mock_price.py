...
def _run_self_tests():
    ...
    assert _response_for_filters(attrs_snapshot_regex), "Anchored SnapshotUsage regex should match"
    ...
    assert _response_for_filters(attrs_snapshot_exact), "Exact SnapshotUsage should match"

    # 2b) UnderBilling must NOT match
    attrs_snapshot_underbilling = [
        {"key":"servicecode","value":"AmazonEC2","operation":"=="},
        {"key":"productFamily","value":"Storage Snapshot","operation":"=="},
        {"key":"usagetype","value":"EBS:SnapshotUsageUnderBilling","operation":"=="},
    ]
    assert _response_for_filters(attrs_snapshot_underbilling) == [], "UnderBilling must not match"
    ...
    def _no_size_block(b):
        return (b in (None, [], {}) or
                (isinstance(b, list) and b and isinstance(b[0], dict) and not any(k in b[0] for k in ("size","volume_size"))))

    assert _no_size_block(inst["values"].get("root_block_device")), "Fixture should omit instance root_block_device size"
    assert _no_size_block(lc["values"].get("root_block_device")),   "Fixture should omit LC root_block_device size"
