...
#   --version, -V        print plancosts version AND terraform version (first line), then exit
# """
...
# # Package version
# try:
#     from plancosts import __version__ as PLANCOSTS_VERSION
# except Exception:
#     PLANCOSTS_VERSION = "dev"
...
# from plancosts.providers.terraform.cmd import terraform_version as _terraform_version
...
# # --version: print plancosts version AND first line of 'terraform -version', then exit
# def _print_versions_and_exit(ctx: click.Context, param: click.Option, value: bool) -> None:
#     if not value or ctx.resilient_parsing:
#         return
#     # plancosts version
#     click.echo(f"plancosts {PLANCOSTS_VERSION}")
#     # terraform version (first line only; non-zero => exit with error)
#     out, err = _terraform_version()
#     if err is None:
#         first_line = (out or "").splitlines()[0] if out else ""
#         if first_line:
#             click.echo(first_line)
#         ctx.exit()
#     # match Go behavior: return error if terraform version fails
#     click.echo(f"Error running terraform -version: {err}", err=True)
#     ctx.exit(1)
...
#     is_eager=True,
#     expose_value=False,
#     callback=_print_versions_and_exit,
#     help="Show plancosts version and Terraform version (first line), then exit.",
# )
# @click.option(
#     "--tfjson",
#     "--tfplan-json",
...
# Default for --tfdir (like Go commit: use pwd and show helpful default text)
DEFAULT_TFDIR = os.getcwd()
...
def _normalize_endpoint(api_url: str | None) -> str:
    """
    Build the GraphQL endpoint from CLI or default.
    Accepts values with or without trailing '/graphql'.
    """
    base = (api_url or PRICE_LIST_API_ENDPOINT or "").rstrip("/")
...
def _price_with_spinner(resources: List[Any], runner: GraphQLQueryRunner, no_color: bool) -> None:
    ...
    if _HAS_RICH_SPINNER:
        ...
        with console.status("Calculating costs…", spinner="dots") as status:
            for i, r in enumerate(resources, start=1):
                name = _name_of_resource(r)
                # explicit keyword for compatibility across rich versions
                status.update(status=f"Calculating costs… ({i}/{total}) {name}")
                get_prices(r, runner)
        return
...
def main(
    ...
    try:
        ...
        # Build GraphQL endpoint: CLI override > env/Config default
        endpoint = _normalize_endpoint(api_url)
        runner = GraphQLQueryRunner(endpoint)
